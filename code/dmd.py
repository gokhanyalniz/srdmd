...
def dmdSpectrum(
    ...
    state : function
        ...
        It should have the signature
            state(i, element="initial", doAvg=False, avgState=None).
...
def filterSpectrum(exponents, modes=None, muMax=0.1, eps=10 ** (-16)):
    ...
    if modes is not None:
        ...
        return exponentsFiltered, picker


def periodicity_indicator(exponents, nh=4, muMax=0.1, eps=10 ** (-16)):
    expF, _ = filterSpectrum(exponents, muMax=muMax, eps=eps)
    nd_eff = len(expF)

    # if there is a real mode
    if np.abs(expF[0].imag) < eps:
        numModes = 1 + nh * 2
        i_start = 1
    else:
        numModes = nh * 2
    ...
    def get_fundFreq(
        ...
        for j in range(nh_eff):
            if lamh[j] < 0:
                exit("negative imaginary part.")
            om_f = om_f + lamh[j]

        if np.abs(om_f) < eps:
            return np.nan

        om_f = (2.0 / (nh_eff * (nh_eff + 1))) * om_f
        return om_f

    def estimate_err(lamh, om_f, nh_eff):
        # no error if nh == 1
        # consider that inconclusive
        if nh_eff <= 1:
            return np.nan
        if not np.isfinite(om_f):
            return np.nan
        ...
        eps_om = eps_om / (nh_eff * om_f**2)
        return eps_om

    # take the positive frequencies
    frequencies = np.imag(expF[:numModes][i_start::2])
    nh_eff = min(nh, len(frequencies))
    fundFreq = get_fundFreq(frequencies, nh_eff)
    indicator = estimate_err(frequencies, fundFreq, nh_eff)
    if not np.isfinite(fundFreq) or np.abs(fundFreq) < eps:
        Tg = np.nan
    else:
        Tg = 2.0 * np.pi / fundFreq

    return indicator, Tg, nh_eff, nd_eff
